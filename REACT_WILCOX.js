 ...
 *  2026/08/26 Added render-state caching to prevent redundant canvas redraws for LM - Charmi
...
function addLogic(id, logic, edges) {
    ...
    edges.forEach((edge) => {
        ...
        if (edgeset[edge].includes(id)) {
            return;
        }

        edgeset[edge].push(id);
...
function setState(key, value) {
    ...
    // console.count(`SETSTATE:${key}`);

    if (nodeset[key].value === value) {
        return;
    }
